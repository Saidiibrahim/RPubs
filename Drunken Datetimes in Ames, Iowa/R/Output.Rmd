...
![image from pexels.com](/Users/ibrahim_saidi/Desktop/GitHub Repositories/RPubs/Drunken Datetimes in Ames, Iowa/images/beer_image.jpeg)
...
![image from pexels.com](/Users/ibrahim_saidi/Desktop/GitHub Repositories/RPubs/Drunken Datetimes in Ames, Iowa/images/nfl_image.jpeg)
```{r part_4}
# read in the football data 
isu_fb <- read_csv("~/Desktop/GitHub Repositories/RPubs/Drunken Datetimes in Ames, Iowa/datasets/isu_football.csv")
...
ba_fb %>% arrange(desc(n)) %>% kbl() %>% kable_material_dark()
...
ba_dates <- ba_dates %>% mutate(mo = month(date, label = TRUE), 
                                yr = year(date) )
...
v13 <- interval(make_date(2013, 04, 15) , make_date(2013, 04, 21), tzone = "America/Chicago" )
v14 <- interval(make_date(2014, 04, 07) , make_date(2014, 04, 13), tzone = "America/Chicago")
...
# count up years 
veishea %>% count(yr) %>% kbl() %>% kable_material_dark()
```
...
count(ba_dates, zero) %>% kbl() %>% kable_material_dark()

# redo ridge with no 0s
ba_dates %>% filter(res > zero) %>% 
...
At a breath alcohol level of 0.16-0.30, a person will experience significant "speech, memory, coordination, attention, reaction time, [and] balance" impairment. Someone's "driving-related skills" and "judgement and decision making" are dangerously impaired, and they may experience "blackouts, vomiting […] and loss of consciousness". BAC of 0.31 or above is life-threatening with "significant risk of death" [(source)](https://magazine.medlineplus.gov/pdf/MLPSpring14_web.pdf). We conclude by looking at the time of day during which the most dangerous levels of alcohol consumption appear in the Ames data. Do the dates, times, and days of week match what one would expect?

While this report has taken a fun and playful tone, it is important to be aware of the seriousness of this issue. According to [research](https://www.niaaa.nih.gov/publications/brochures-and-fact-sheets/alcohol-facts-and-statistics), nearly 2,000 college students die each year from alcohol-related injuries.
```{r part_10}
# filter the ba_dates data to contain only those with the most dangerous result
ba_dates %>% filter(res >= 0.31) %>% kbl() %>% kable_material_dark()
